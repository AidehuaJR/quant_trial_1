...
type LiveQuote = { symbol: string; timestamp: string; lastPrice: string; currency: string };
type TossCandle = {
  timestamp: string;
...
function intervalLabel(language: Language, minutes: Interval) { return minutes === 1440 ? rangeLabel(language, "1일") : minutes < 60 ? `${minutes}${chartText(language,"분")}` : `${minutes/60}${chartText(language,"시간")}`; }
...
function sampleBars(base: number, code: string, range: RangeKey, minutes: Interval): Bar[] {
  ...
  const result: Bar[] = [];
  let close = base * (range === "전체" ? .28 : range === "1년" ? .72 : range === "3개월" ? .86 : .96);
  let index = 0;
  ...
  // A short, bounded fallback keeps low-powered phones responsive while the
  // real Toss minute candles are loading or temporarily unavailable.
  return minutes <= 5 ? result.slice(-240) : result;
}

function normalizeTossCandles(items: TossCandle[]): Bar[] {
...
export default function MarketChart({ name, code, price, entry, stop, target, language }: Props) {
  ...
  async function fetchLiveQuote(signal?: AbortSignal) {
    try {
      ...
      const payload = await response.json() as { result?: LiveQuote[] };
      const quote = payload.result?.find(item => item.symbol === code) ?? payload.result?.[0];
      const nextPrice = Number(quote?.lastPrice);
      if (!quote || !Number.isFinite(nextPrice)) throw new Error("Quote response did not contain a valid price");
      setLivePrice(nextPrice);
      setLiveTimestamp(quote.timestamp);
      setLastUpdated(new Date(quote.timestamp).toLocaleTimeString(localeFor[language], { hour12: false, timeZone: "Asia/Seoul" }));
      setFeedError(false);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const bucket = Math.floor(quoteTime / intervalSeconds) * intervalSeconds as UTCTimestamp;
    const previous = liveBarRef.current;
    const next: Bar = previous?.time === bucket
      ? {
          ...previous,
  ...
  function refreshChart() {
    setRefreshing(true);
    setDetail(null);
    void fetchLiveQuote().finally(() => window.setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const chart = createChart(container.current, {
      ...
      timeScale: { borderColor: "#e5ebe8", timeVisible: range !== "전체", secondsVisible: false, rightOffset: 3, barSpacing: range === "전체" ? 1 : range === "1일" ? 6 : 4 },
    ...
    if (range === "전체") chart.timeScale().fitContent();
    else if (bars.length > 30) chart.timeScale().setVisibleLogicalRange({ from: bars.length - 30.5, to: bars.length - .25 });
  ...
  const displayedTimestamp = liveTimestamp
    ? new Intl.DateTimeFormat(localeFor[language], { timeZone: "Asia/Seoul", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false }).format(new Date(liveTimestamp))
    : "연결 중…";
